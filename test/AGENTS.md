...
| `known-gaps.tsv` | the residual gaps, **per runtime**, pinned as an exact set: `runtime`, `kind`, `key`, `count`, `note`. The `runtime` column holds `ts`, `go` or `rust`, and each suite reads only its own rows. |
...
- The corpus itself (`abnf-corpus/`) is fetched, never committed. All
  three suites **fail** when it is absent; none skips.

## Format — `spec/*.tsv`

`spec/*.tsv` holds the cross-runtime conformance fixtures. Every runtime
runs every fixture here, so a change affects all three languages — edit
with that in mind. Same convention as
[`@tabnas/parser`'s `test/spec/`](https://github.com/tabnas/parser/tree/main/test).

These fixtures are what keeps `go/` and `rs/` honest against `ts/`. The
older `go/leftrec_test.go` and `go/rfc3986_test.go`, and their Rust
twins, mirror the TS suite by hand, which catches nothing when only one
side changes.
...
loader, in three languages written to behave identically.
...
identical across TS, Go and Rust.
...
shared package, so the three loaders cannot drift from each other
either.

An `ERROR:` row's message is compared EXACTLY, in every runtime, through
...
- Prefer adding a fixture here over a one-off in-language assertion when
  a case is expressible as grammar → output. That is what keeps the
  three runtimes honest against each other.
- A new case must pass in EVERY runtime: run `go test ./...` (from
...
- `ts/` is canonical. Author expectations from the TS side, then make Go
  and Rust match — not the other way round.
