# libtabnasabnf — the abnf parser as a C ABI

<!-- tabnas-clib-template: v1 — stamped by admin tasks/adopt-clib.sh;
     edit the template and re-stamp, not this file. -->

The abnf format parser as a C shared library, so languages with no
tabnas port can parse and validate abnf input. This is one of the
...
| Function | Returns |
|---|---|
| `tabnas_version()` | `{"ok":true,"lib":"libtabnasabnf","format":"abnf","template":"v1"}` |
...
// link against libtabnasabnf; every call returns a JSON []u8 to free with
// c.tabnas_free.
```

## Format notes

ABNF validation is accept/reject only, and `ParseAbnf` drives a package-global singleton parser — every handle in this process serializes on one shared lock, so extra handles buy no concurrency.
