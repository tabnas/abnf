...
//
// libtabnasabnf is the abnf format parser as a C shared library, one of
...
import (
	...
	plug "github.com/tabnas/abnf/go"
)

const (
	templateVersion = "v1"
	libName         = "libtabnasabnf"
	formatName      = "abnf"
	valueOut        = false
)
...
// newParser builds one engine with the abnf grammar installed
...
func newParser() (parseFn, error) {
	return func(src string) (any, error) { sharedMu.Lock(); defer sharedMu.Unlock(); _, err := plug.ParseAbnf(src); return nil, err }, nil
}
...
// parseWith answers whether src is in the abnf language — and,
