...
describe('rfc3986', () => {
  ...
  describe('remaining LL(k) limitations', () => {
    ...
    // (Removed: an `it.skip('placeholder …')` with an EMPTY body used to sit
    // here. It asserted nothing, so un-skipping it would have passed
    ...
    // content; the concrete unhandled shapes are covered by the two `.skip`
    ...
    //                      FIRST(A) ∩ FIRST(C) ≠ ∅, and B is not a terminal
    //                      (so there is no single token to peek for).
    // The rewriter requires `D` to be a term / regex element; detection
    // skips this shape with no rewrite.
